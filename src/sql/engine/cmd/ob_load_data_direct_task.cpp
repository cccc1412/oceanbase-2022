...
ObAsyncTask *ObLoadDataDirectTask::deep_copy(char *buf,
  ...
  if (NULL == buf || buf_size < (sizeof(*task))) {
    ...
    task = new (buf)
        ObLoadDataDirectTask(index_, ctx_, stmt_, offset_, end_, stage_process_,
...
int ObLoadDataDirectTask::process() {
  ...
  if (OB_ISNULL(load_direct =
                    OB_NEWx(ObLoadDataDirectDemo, (&ctx_.get_allocator())))) {
    ret = OB_ALLOCATE_MEMORY_FAILED;
    LOG_WARN("allocate memory failed", K(ret));
  } else if (OB_FAIL(load_direct->init(index_, stmt_, offset_, end_,
                                       stage_process_, dispatcher_,
                                       sstable_writer_))) {
