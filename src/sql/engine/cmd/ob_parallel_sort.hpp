...
void radix_sort(common::ObVector<T *> &list, Compare compare, int64_t min_id0, int32_t min_id1, int64_t max_id0, int32_t max_id1) {

  size_t size = list.size();
  if (size < RadixTraits<T>::kRadixThreshold) {
    ...
    // 如果数据跨度范围比较大，那么进行快排
    if (id0_round + id1_round > RadixTraits<T>::nBytes / 2) {
      std::sort(list.begin(), list.end(), compare);
...
template <typename T> int ObFragmentReaderV2<T>::prefetch() {
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    if (nullptr == buf_) {
      if (OB_ISNULL(buf_ =
                        static_cast<char *>(allocator_.alloc(buf_size_ / 3)))) {
        ret = common::OB_ALLOCATE_MEMORY_FAILED;
...
int ObMemorySortRound<T, Compare>::add_item_reuse(const T &item) {
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    min_id0 = std::min(min_id0, item.value.id0);
    min_id1 = std::min(min_id1, item.value.id1);
    max_id0 = std::max(max_id0, item.value.id0);
    max_id1 = std::max(max_id1, item.value.id1);
...
int ObMemorySortRound<T, Compare>::build_fragment_reuse() {
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    int64_t start = common::ObTimeUtility::current_time();
    radix_sort(item_list_, *compare_, min_id0, min_id1, max_id0, max_id1);
    if (OB_FAIL(compare_->result_code_)) {
      ret = compare_->result_code_;
    } else {
      const int64_t sort_fragment_time =
