...
struct ObExternalSortConstant {
  ...
  static const int64_t MIN_MEMORY_LIMIT =
...
template <typename T> class ObFragmentWriterV2 {
public:
  ObFragmentWriterV2();
...
ObFragmentWriterV2<T>::ObFragmentWriterV2()
  : is_inited_(false), is_first_write_(true), 
    buf_(NULL),compress_buf_(NULL), 
    buf_size_(0), expire_timestamp_(0),
...
template <typename T>
int ObFragmentWriterV2<T>::open(const int64_t buf_size,
  ...
  if (OB_UNLIKELY(is_inited_)) {
    ...
    } else if (NULL == (compress_buf_ = static_cast<char *>(allocator_.alloc(
...
template <typename T> int ObFragmentWriterV2<T>::write_item(const T &item) {
  int ret = common::OB_SUCCESS;
  if (OB_UNLIKELY(!is_inited_)) {
...
template <typename T> int ObFragmentWriterV2<T>::sync() {
  int ret = common::OB_SUCCESS;
...
public:
  ...
  void init_decompress_buffer(common::ObArenaAllocator &allocator) {
    if(decompress_buf_ == NULL) {
      decompress_buf_ = static_cast<char*>(allocator.alloc(ObExternalSortConstant::DECOMPRESS_BUFFER_SIZE));
      //decompress_buf_ = (char*)ob_malloc(ObExternalSortConstant::DECOMPRESS_BUFFER_SIZE);
    }
  }
...
//template<typename T>
//thread_local char* ObMacroBufferReader<T>::decompress_buf_ = NULL;
...
ObMacroBufferReader<T>::ObMacroBufferReader()
  : buf_(NULL) , decompress_buf_(NULL), 
  buf_pos_(0), buf_len_(0), buf_cap_(0)
{
...
template <typename T> int ObMacroBufferReader<T>::read_item(T &item, int idx) {
  ...
  if (OB_SUCC(ret)) {
    if (buf_pos_ == buf_len_) {
      ret = common::OB_EAGAIN;
    } else if (OB_FAIL(item.datums_[idx].deserialize(buf_, buf_len_, buf_pos_))) {
...
template<typename T>
void ObMacroBufferReader<T>::assignv2(const int64_t buf_pos, const int64_t buf_cap)
{
  buf_pos_ = buf_pos;
  buf_cap_ = buf_cap;
  buf_len_ = 0;
...
template <typename T> int ObFragmentReaderV2<T>::prefetch_all(bool is_open_prefetch) {
  int ret = OB_SUCCESS;
  for(int i = 0; i < count_; i++) {
...
template <typename T> int ObFragmentReaderV2<T>::prefetch(int idx, bool is_open_prefetch) {
  int ret = common::OB_SUCCESS;
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
    STORAGE_LOG(WARN, "ObFragmentReaderV2 has not been inited", K(ret));
    ...
    if (nullptr == buf_[idx]) {
      if (OB_ISNULL(buf_[idx] = static_cast<char *>(allocator_.alloc(buf_size_)))) {
        ret = common::OB_ALLOCATE_MEMORY_FAILED;
        STORAGE_LOG(WARN, "fail to allocate memory", K(ret));
      }
    }
...
template <typename T> int ObFragmentReaderV2<T>::wait(int idx) {
  int ret = common::OB_SUCCESS;
  const int64_t wait_cursor = (handle_cursor_[idx] + 1) % MAX_HANDLE_COUNT;
  int64_t timeout_ms = 0;
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
    STORAGE_LOG(WARN, "ObFragmentReaderV2 has not been inited", K(ret));
    ...
    macro_buffer_readers_[idx].assign(0, buf_size_,
                                file_io_handles_[idx][wait_cursor].get_buffer());
  }
  return ret;
}
...
template <typename T> int ObFragmentReaderV2<T>::wait_all() {
  int ret = common::OB_SUCCESS;
  for(int i = 0; i < count_; i++) {
    const int64_t wait_cursor = (handle_cursor_[i] + 1) % MAX_HANDLE_COUNT;
    int64_t timeout_ms = 0;
    if (OB_UNLIKELY(!is_inited_)) {
      ret = OB_NOT_INIT;
      STORAGE_LOG(WARN, "ObFragmentReaderV2 has not been inited", K(ret));
...
template <typename T> int ObFragmentReaderV2<T>::pipeline(int idx) {
  ...
  } else if (OB_FAIL(prefetch(idx, false))) {//perfetch next next buffer,里面解析next buffer的header
    STORAGE_LOG(WARN, "fail to prefetch data", K(ret));
  }
  return ret;
}
    ...
    for(int idx = 0; idx < count_; idx++) {
      if (OB_FAIL(macro_buffer_readers_[idx].read_item(curr_item_, idx))) {
        if (common::OB_EAGAIN == ret) {
          ...
          } else if (OB_FAIL(macro_buffer_readers_[idx].read_item(curr_item_, idx))) {
